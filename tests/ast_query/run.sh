#!/usr/bin/env bash
# AstQuery tests. Dependencies: g++, bash, diff and timeout. Nothing external.
#
#   ./run.sh          run everything, one line per case
#   ./run.sh -u       rewrite the expected/ files with the current output
#
# Every file in cases/*.hd is scanned, parsed and then asked what it declares:
# its imports, its functions, its types and its global variables. The answers
# are compared against expected/<case>.txt. Read the diff before committing it:
# -u accepts whatever AstQuery produces, bugs included.
#
# A case that does not parse prints its diagnostics instead, which is the phase
# gate AstQuery relies on written down as a golden.
#
...
root=../..
build=${TMPDIR:-/tmp}/haard-ast-query-tests
update=0
...
if ! err=$(g++ -std=c++20 -I"$root/src" -o "$build/query" \
        query.cpp "${HAARD_OBJECTS[@]}" 2>&1); then
    printf '%sERROR%s failed to build query\n' "$red" "$reset"
    printf '%s\n' "$err"
    exit 1
fi
...
for case in "${cases[@]}"; do
    ...
    got=$(timeout 5 "$build/query" "$case" 2>&1)
