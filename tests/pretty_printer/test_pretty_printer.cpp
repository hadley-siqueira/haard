...
// import std.io as io
//  0      1 2 3  4  5
void test_import(Module& module, AstBuilder& builder) {
    ...
    u32 root = builder.make_module();

    builder.add_child(root, 0, import);

    check("import with a path and an alias", print(module),
          "import std.io as io\n");
}

// let x : i32 = a + b
//  6  7 8  9  10 11 12 13
void test_let_declaration(Module& module, AstBuilder& builder) {
    ...
    builder.add_child(root, 0, builder.make_let_declaration(6, binding));

    check("let declaration with a type and an expression", print(module),
          "let x : i32 = a + b\n");
}

// @count : u32
//  14 15 16 17
void test_param(Module& module, AstBuilder& builder) {
    ...
    builder.add_child(root, 0, builder.make_param(14, name, type, 0));

    check("param", print(module), "@count : u32\n");
}
...
void test_param_with_a_default(Module& module, AstBuilder& builder) {
    ...
    builder.add_child(root, 0, builder.make_param(51, name, type, value));

    check("param with a default", print(module), "@limit : i32 = 80\n");
}

// let p = io::println
//  18 19 20 21 22 23
void test_scope_with_an_alias(Module& module, AstBuilder& builder) {
    ...
    builder.add_child(root, 0, builder.make_let_declaration(18, binding));

    check("scope with an alias", print(module), "let p = io::println\n");
}
...
// forms apart
void test_scope_without_an_alias(Module& module, AstBuilder& builder) {
    ...
    builder.add_child(root, 0, builder.make_let_declaration(24, binding));

    check("scope with no alias", print(module), "let q = ::println\n");
}
...
void test_parenthesis(Module& module, AstBuilder& builder) {
...
void test_no_parenthesis_is_added(Module& module, AstBuilder& builder) {
...
void test_binding_without_type(Module& module, AstBuilder& builder) {
    ...
    check("a binding with no type and no expression", print(module),
          "let x\n");
}
...
void test_module_with_three_children(Module& module, AstBuilder& builder) {
    u32 root = builder.make_module();
    u32 last = 0;

    last = builder.add_child(root, last, builder.make_let_declaration(6,
        builder.make_binding(builder.make_binding_name(
            builder.make_identifier(7)), 0, 0)));

    last = builder.add_child(root, last, builder.make_const_declaration(6,
        builder.make_binding(builder.make_binding_name(
            builder.make_identifier(15)), 0, 0)));

    builder.add_child(root, last, builder.make_let_declaration(6,
        builder.make_binding(builder.make_binding_name(
            builder.make_identifier(11)), 0, 0)));

    check("module joins its children with a line break", print(module),
          "let x\nconst count\nlet a\n");
}
...
void test_unknown_kind(Module& module, AstBuilder& builder) {
    AstNode node;
    u32 root = builder.make_module();

    node.set_kind(AST_UNKNOWN);
    builder.add_child(root, 0, module.get_ast()->push(node));

    check("a kind with no printer is reported in the output", print(module),
...
void test_sentinel_is_untouched(Module& module, AstBuilder& builder) {
    u32 root = builder.make_module();

    builder.add_child(root, 0, builder.make_let_declaration(6,
        builder.make_binding(builder.make_binding_name(
            builder.make_identifier(7)), 0, 0)));

    check("building a tree does not touch the sentinel",
          state_of(module.get_ast(), 0),
...
void test_out_of_range_is_the_sentinel(Module& module, AstBuilder& builder) {
    u32 root = builder.make_module();

    builder.add_child(root, 0, builder.make_identifier(7));

    check("an index past the end gives back the sentinel",
          state_of(module.get_ast(), 9999),
          "kind=0 token=0 sibling=0 children=0");
}
...
void test_long_list_of_children(Module& module, AstBuilder& builder) {
    ...
    for (u32 i = 0; i < total; ++i) {
        last = builder.add_child(root, last, builder.make_identifier(7));
    ...
    u32 walked = 0;
    u32 child = ast->get_node(root)->get_children();
...
void test_empty_ast() {
    ...
    check("an empty ast prints nothing", print(module), "<nothing to print>");
}
...
int main(int argc, char* argv[]) {
    ...
    // one test per Module: the ast is append only and each test builds a
    // tree of its own, so they cannot share one
