...
| `a_literal_says_which_constructor_it_meant` | record 0037 carrying `char*` → `String` since 2026-09-08: a written literal reaching a class is a **construction**, and the golden prints the `init` it means at each of the four places — a binding, an argument, a return and an assignment. The class is not called `String`, which is the point: a literal reaches any class that says how to build one, while the one diagnostic on the golden is a `char*` **value** reaching the same class and being refused, because a value is still record 0023's named entry and that entry knows one name |
