#!/usr/bin/env bash
# SugarLowerer tests. Dependencies: g++, bash, diff and timeout. Nothing external.
#
...
# from, the sources, and a 'table.tbl' when the case needs a prelude to give
# it a String. show.cpp compiles it and prints the entry module's tree back
# as Haard, so the golden is the sugar taken apart. Read the diff before
# committing it: -u accepts whatever it produces, bugs included.
#
...
root=../..
build=${TMPDIR:-/tmp}/haard-sugar-tests
update=0
...
if ! err=$(g++ -std=c++20 -I"$root/src" -o "$build/show_case" \
        show.cpp "${HAARD_OBJECTS[@]}" 2>&1); then
...
for directory in "${cases[@]}"; do
    ...
    got=$(timeout 5 "$build/show_case" "cases/$name" 2>&1)
