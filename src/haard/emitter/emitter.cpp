...
bool Emitter::emit(std::ostream& stream) {
    ...
    // record 0030: the native functions of 'std.low_io' are written in terms
...
void Emitter::emit_statement(u32 module, u32 node) {
    ...
    // the name is the label's child: its own token is the word 'label',
    // which is what this wrote until 2026-09-24 -- so every 'goto' pointed at
    // a label C++ never saw
    case AST_LABEL:
        out << text_of(module, child_of(module, node, 0)) << ":;\n";
        return;
...
void Emitter::define_closure(u32 module_index, u32 node) {
    ...
    if (back != 0 && result != types->builtin(BUILTIN_VOID)) {
        // what the sugar built in front of the one expression -- the String
        // a '${}' in it becomes -- and then the expression, given back
        for (u32 before = child_of(module_index, query.get_block(node), 0);
             before != 0 && before != back;
             before = compilation->get_module(module_index)->get_ast()
                          ->get_node(before)->get_sibling()) {
            emit_statement(module_index, before);
        }
